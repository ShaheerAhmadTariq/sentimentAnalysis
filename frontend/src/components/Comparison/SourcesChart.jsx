...
const SourcesChart = ({ brandKey, currentDate, prevDate, data }) => {
  const [series, setSeries] = useState([data.Postive, data.Negative, data.Neutral]);
  ...
  const [options, setOptions] = useState({
    labels: [[`Positive: ${((data.Positive /(data.Positive+data.Negative+data.Neutral))*100).toFixed(1)}`],[`Negative: ${((data.Negative /(data.Positive+data.Negative+data.Neutral))*100).toFixed(1)}`],[`Neutral: ${((data.Neutral /(data.Positive+data.Negative+data.Neutral))*100).toFixed(1)}`]],
    chart: {
      type: "donut",
      hight: 100,
      width: 100,
